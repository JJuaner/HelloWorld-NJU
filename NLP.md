语言是什么？分类：自然语言，程序语言
自然语言分类：基于形态，基于句法
自然语言为什么难？语言多样动态，歧义，常识
中文为什么难？缺少理论缺失资源 分词，词法句法语义都难
形式语言-CFG-pCFG
句法分析器（自顶向下，自底向上）

自然语言处理vs自然语言理解vs计算语言学
自然语言处理两类方法及优劣
自然语言处理发展历程
自然语言处理任务：分析（词法，句法，语义），生成，应用
词法分析（分词/形态变化，词性标注/实体识别）
句法分析（结构，依存）
语义分析（实体关系，逻辑）
图5.9

词法分析：分词，形态还原，词性标注，命名实体识别
分词：
切分歧义：交集歧义，组合歧义，混合歧义
分词词典和分词规则：方法：正向最大匹配/反向最大匹配，双向最大（交集性歧义）+消歧规则，正向最大反向最小（组合歧义）消歧，全切分

语义分析目的
包括词义分析，句义分析，篇章分析
词义的表示：义项，语义类，义素
句法义分析：1.逻辑形式 2.论旨角色/格语法 基于动词给出句子其他成分和它的关系/语义规则：S->M+P P->V+C1+C2 C->K+NP
...
高维-->特征工程/特征选择（最重要最相关最有分别力的特征/目标是表示词表示文档，这里或者说是选词）：stopword，freq，mutual互信息，x2卡方检测，tfidf
正交--->分布式基于相似度的表示


潜在语义分析/LSI：词-文档的共现矩阵X:n×m 
问题：非常大/更低维的表示 
...
开放题（2+2+2+2）
词性标注(hmm，lstm+CRF)任务扩展：词法分析，序列标注

语言模型P(W)（ngram，线性语言模型，神经网络语言模型）ngramVSFNNLM
文本表示：文本转为数字表示
文本分类：朴素贝叶斯
机器翻译：朴素贝叶斯生成模型，
...
词袋模型：文本表示,文本分类
统计计数：从训练数据中估计参数，极大似然估计，相对频率：语言模型马尔可夫，文本分类贝叶斯，词性标注hmm模型，句法分析pcfg
