...
统计机器翻译，能一定程度上能自动从数据中挖掘翻译规则，流程复杂，性能有限
神经网络机器翻译：直接把句子看成单词序列，序列到序列的翻译方式，利用注意力机制动态获取信息（不需要建模规则组合，不依赖语料库中学习的又噪音规则）Bi-directional RNN + Attention
...
注意力机制qkv 
q和k计算相似度分数（点积，general。cancat）-通过softmax后得到权重-v权重相乘累加
...
HMM 生成模型/模型参数：三个表格
三个基本问题
1.给定模型参数，计算观测序列的概率/这个模型/参数生成观测序列的概率、   给定模型，生成模型，观测序列的生成概率
